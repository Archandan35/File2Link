...
async def save_token(token, msg_id, file_name, file_size):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "INSERT OR REPLACE INTO files VALUES (?,?,?,?)",
            (token, msg_id, file_name, file_size)
        )
        await db.commit()
...
async def get_token_entry(token):
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            "SELECT msg_id, file_name, file_size FROM files WHERE token=?",
            (token,)
        ) as cursor:
...
def telethon_stream_thread(
    ...
    async def _run():
        client = TelegramClient(
            StringSession(SESSION_STRING), API_ID, API_HASH, loop=loop
        )
        try:
            await client.connect()
            logger.info(f"✅ Thread Telethon connected msg_id={msg_id}")

            tl_msg = await client.get_messages(CHANNEL_ID, ids=msg_id)
            if tl_msg is None:
                logger.error(f"❌ Message {msg_id} not found in channel")
                chunk_q.put(None)
                return
            ...
            async for chunk in client.iter_download(
                ...
                if stop_event.is_set():
                    logger.info("Stop event set — ending stream")
                    break
                chunk_q.put(chunk)

        except Exception as e:
            logger.error(f"Telethon thread error: {e}", exc_info=True)
        finally:
            chunk_q.put(None)
...
async def stream_handler(request: web.Request) -> web.StreamResponse:
    ...
    logger.info(f"▶️ Streaming: {file_name} offset={offset}")

    chunk_q    = queue.Queue(maxsize=16)
    stop_event = threading.Event()

    t = threading.Thread(
        target=telethon_stream_thread,
        args=(msg_id, offset, chunk_q, stop_event),
        daemon=True
    )
    t.start()
    ...
    try:
        ...
        logger.info(f"✅ Stream done: {file_name}")
...
async def process_batch(
    ...
    for i, message in enumerate(messages, 1):
        try:
            ...
            # Save to DB immediately after forwarding
            ...
            logger.info(f"✅ {i}/{total}: {file_name} → channel msg {channel_msg_id}")
    ...
    text += f"📦 Total: {total_size_mb:.1f} MB\n"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "👋 *Welcome!*\n\n"
        "Forward any file or multiple files:\n"
        "⚡ Instant stream and download links\n"
        "📦 Batch: 2 to 100 files at once\n"
...
async def on_startup(app_obj):
    scheduler.start()
    await init_db()
    logger.info(f"✅ Bot ready! BASE_URL={BASE_URL} CHANNEL={CHANNEL_ID}")
